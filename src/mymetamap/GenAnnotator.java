...
public class GenAnnotator implements MyFeature{
		  ...
		  public WNTagger tagger;
		  public BNTagger btagger;
		  ...
		  public GenAnnotator(String serverHostname, int serverPort, String path) throws Exception{
			  
			  // taggers
			  tagger = new WNTagger();
			  btagger = new BNTagger();
...
//					   annotateTreeMM(this.parse,num,myoutput,myoptions);
//				   }				   	
//			   }			   
//		  }
		  ...
		  public void processLine(String sample) throws Exception{
			   ...
			   // annotate NPs and verbs
			   // annotateTreeMM(this.parse,0,myoutput,myoptions); // MetaMap
			   // annotateTreeWN(this.parse,0,myoutput); // WordNet
			   // annotateTreeBN(this.parse,0,myoutput); // BabelNet			   
		  }
		   
		  
		  // Display NP and Verb annotations (MetaMap)
		  public void annotateTreeMM(Tree myparse, int num, PrintStream myoutput, List<String> myoptions) throws Exception
		  { 
		  ...
		  // Display NP and Verb annotations (WordNet)
		  public void annotateTreeWN(Tree myparse, int num, PrintStream myoutput) throws Exception
		  { 
				Set<Tree> sub = myparse.subTrees();
				filterNPs(sub);
				for (Tree tree: sub){					
					...
					if (tree.label().value().matches("(VB|VBN|VBD|VBG|VBP|VBZ)")){ // base(=act)+part.(=pass)
						...
						ArrayList<TaggedWord> words = tree.taggedYield();
						for (TaggedWord wor: words){	
							vp = vp + " " + wor.toString().replaceAll("/.*", "");
							tagger.exploreVerb(wor.word()); //WordNet annotation
						}
						System.out.println("Verb phrase: " + vp);						
						System.out.println("Head verb : " + head.determineHead(tree));
					}
		  ...
		  // Display NP and Verb annotations (BabelNet)
		  public void annotateTreeBN(Tree myparse, int num, PrintStream myoutput) throws Exception
		  { 
				Set<Tree> sub = myparse.subTrees();
				filterNPs(sub);
				for (Tree tree: sub){					
					...
					if (tree.label().value().matches("NP")){ // all NPs
						...
						for (Word wor: words){
							np = np + " " + wor.toString().replaceAll("/.*", "");
							btagger.exploreWord(wor.word()); //BabelNet annotation
						}
						System.out.println("Noun phrase: " + np);
						System.out.println("Head noun : " + head.determineHead(tree));
					}					
